...
// 1.2. Pawn structure
constexpr int16_t Doubled = -100, Isolated = -80, Backward = -100, wpassed = 100, cpassed = 50, Island = -30,
                  Center = 100, hole = -50;
// 1.3. Mobility
...
// 1.3. King safety
constexpr int16_t pawnShield = 20, pawnStorm = -12, KingTropism = 5;
// 1.4. Space
...
AllPhases inline int16_t material(
    const chess::Position &pos) {  // Precompute piece counts (avoid multiple function calls)
    int pieceCount[10] = {
...
Middlegame int16_t isolated(const chess::Position &pos) {
    chess::Bitboard P = pos.pieces(chess::PieceType::PAWN, chess::Color::WHITE),
                    p = pos.pieces(chess::PieceType::PAWN, chess::Color::BLACK),
                    adjP = (P << 1 | P >> 1) & 0x6F6F6F6F6F6F6F6F, adjp = (p << 1 | p >> 1) & 0x6F6F6F6F6F6F6F6F,
                    Pi = P & ~adjP, pi = p & ~adjp;
...
Middlegame int16_t holes(const chess::Position &pos) {
    ...
    chess::Bitboard whiteDef =
        ((whitePawns << 8) | (whitePawns >> 8)) | ((whitePawns << 1) & FILE_A_MASK) | ((whitePawns >> 1) & FILE_H_MASK);

    // Defensive coverage for black
    chess::Bitboard blackDef =
        ((blackPawns << 8) | (blackPawns >> 8)) | ((blackPawns << 1) & FILE_A_MASK) | ((blackPawns >> 1) & FILE_H_MASK);
...
Middlegame int16_t center(const chess::Position &pos) {
...
int16_t pawn_storm(const chess::Position &board) {
    using namespace chess;

    auto eval_storm = [&](Color defender) {
        Square kingSq = board.kingSq(defender);
        int fileK = kingSq.file();
        int rankLimit = (defender == Color::WHITE) ? 5 : 2;

        Bitboard attackers = board.pieces(PieceType::PAWN, ~defender);
        int score = 0;

        while (attackers){
            chess::Square sq=attackers.pop();
            int fileDiff = std::abs(sq.file() - fileK);
            int rank = sq.rank();

            if (fileDiff <= 2) {
                // Enemy pawns close to the king and advancing
                if ((defender == Color::WHITE && rank >= rankLimit) ||
                    (defender == Color::BLACK && rank <= rankLimit)) {
                    score += pawnStorm * (3 - fileDiff); // closer files = higher penalty
                }
            }
        }
    ...
    wEval.Storm=eval_storm(Color::WHITE);
    bEval.Storm=eval_storm(Color::BLACK);
    return wEval.Storm-bEval.Storm;
}

Middlegame int16_t king_tropism(const chess::Position &board) {
    using namespace chess;
...
Middlegame inline int16_t king_safety(const chess::Position &board) { return pawn_shield(board) + pawn_storm(board); }
// 2.6. Space
...
Endgame bool draw(const chess::Position &board) {
    ...
    // KvK
    if (totalNonPawnPieces == 0 && totalPawns == 0) return 0;

    // KBvK or KNvK
    if (totalNonPawnPieces == 1 && totalPawns == 0 &&
        (pieceCount[1] == 1 || pieceCount[2] == 1 || pieceCount[6] == 1 || pieceCount[7] == 1))
        return 1;
    ...
    if (totalNonPawnPieces == 2 && totalPawns == 0) {
        ...
        if (pieceCount[2] == 2) {
            ...
            if (chess::Square::same_color(first, second)) return 1;
        }
        ...
        // KNNvK
        if ((pieceCount[1] == 2 && pieceCount[6] == 0) || (pieceCount[1] == 0 && pieceCount[6] == 2)) return 1;

        // KBNvK
        if ((pieceCount[1] == 1 && pieceCount[2] == 1) || (pieceCount[6] == 1 && pieceCount[7] == 1)) return 1;

        // KNvKN
        if (pieceCount[1] == 1 && pieceCount[6] == 1) return 1;
    }
 ...
 */
AllPhases int16_t psqt_eval(const chess::Position &board) {
    int score = 0;
    ...
    return wEval.PSQT-bEval.PSQT;  // Negate for Black
}
// 3. Main evaluation
int16_t eg(const chess::Position &pos) {
    if (draw(pos)) return 0;
    int score = space(pos) + tempo(pos) + material(pos) + passed(pos) + psqt_eval(pos);
    return score;
}
int16_t mg(const chess::Position &pos) {
    return material(pos) + space(pos) + tempo(pos) + mobility(pos) + king_safety(pos) + psqt_eval(pos);
}
...
void trace(const chess::Position &pos) { traceEvaluationResults("Handcrafted", eval(pos)); }
