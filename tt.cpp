...
#else
  // Portable fallback (rare): fall back to modulo if no 128-bit or _umul128
  // available. This is only used on very uncommon toolchains; primary
  // implementations above avoid division.
  return hash % buckets;
#endif
}
...
void TranspositionTable::store(uint64_t hash, chess::Move best, int16_t score,
                               int8_t depth, TTFlag flag) {
