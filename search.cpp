...
Value qsearch(Board &board, Value alpha, Value beta, search::Session &session,
  ...
  if (((session.nodes & 2047) == 0 &&
       session.tm.elapsed() >= session.tm.optimum()) ||
...
Value doSearch(Board &board, int depth, Value alpha, Value beta,
  ...
  if (depth >= 3 && !inCheck && ply > 0 && staticEval >= beta &&
      !is_win(beta)) {
    int R = 2 + depth / 6 + std::min(2, depth / 10);
    board.doNullMove();
    Value score = doSearch(board, depth - 1 - R, -beta, -beta + 1, session,
                           ply + 1, Move::none());
    board.undoMove();
    if (score == VALUE_NONE)
      return VALUE_NONE;
  ...
  if (depth >= 5 && !inCheck && !is_win(beta) &&
      std::abs(beta) < VALUE_TB_WIN_IN_MAX_PLY) {
    Value probCutMargin = Value(200 + 100 * (depth - 5));
    if (staticEval >= beta + probCutMargin) {
      Value v =
          doSearch(board, depth - 2, beta - 1, beta, session, ply, prevMove);
      if (v == VALUE_NONE)
        return VALUE_NONE;
      if (v >= beta)
        return v;
      ...
      if (wdl != engine::tb::TB_ERROR) {
        ...
        TTFlag b = wdl < -drawScore  ? TTFlag::UPPERBOUND
                   : wdl > drawScore ? TTFlag::LOWERBOUND
                                     : TTFlag::EXACT;
        if (b == TTFlag::EXACT ||
            (b == TTFlag::LOWERBOUND ? value >= beta : value <= alpha)) {
          tt.store(hash, Move::none(), value_to_tt(value, ply),
                   std::min(MAX_PLY - 1, depth + 6), b);
          return value;
        }
  ...
  if (depth >= 12 && ttMove.is_ok() && !inCheck && ply > 0 && entry &&
      entry->getDepth() >= depth - 4 &&
      entry->getFlag() != TTFlag::UPPERBOUND && alpha != beta - 1 &&
      !is_win(beta)) {
    ...
    if (moves.size() <= 5) {
      ...
      for (size_t si = 0; si < moves.size() && singular; ++si) {
        ...
        Value v =
            doSearch(board, r, sBeta - 1, sBeta, session, ply + 1, moves[si]);
        board.undoMove();
        if (v == VALUE_NONE) {
          board.undoMove();
          singular = false;
  ...
  for (size_t i = 0; i < moves.size(); ++i) {
    ...
    if (!inCheck && !isCapture && !givesCheck && depth <= 2 && ply > 0 &&
        movesSearched > 0) {
    ...
    if (!inCheck && !isCapture && !givesCheck && depth <= 2 &&
        movesSearched > 3 + 2 * depth &&
        std::abs(alpha) < VALUE_TB_WIN_IN_MAX_PLY)
      continue;

    // SEE pruning for losing captures at shallow depths
    if (!inCheck && isCapture && !givesCheck && depth <= 2 &&
        movesSearched > 0 && move.type_of() != PROMOTION &&
        std::abs(alpha) < VALUE_TB_WIN_IN_MAX_PLY) {
      if (movepick::see(board, move) < 0)
        continue;
    }
    ...
    if (movesSearched >= 2 && depth >= 3) {
      if (!isCapture && !givesCheck) {
        ...
        if (history > 0)
          ...
          reduction++;
        if (move == session.killerMoves[ply][0] ||
            move == session.killerMoves[ply][1])
          reduction--;
        if (prevMove.is_ok() &&
            move == session.counterMoves[prevMove.from_to()])
          reduction--;
    ...
    if (movesSearched == 0 || reduction == 0) {
      score = doSearch(board, depth - 1 + ext, -beta, -alpha, session, ply + 1,
                       move);
      if (score == VALUE_NONE) {
        board.undoMove();
      ...
      if (score > alpha && reduction) {
        ...
        score = doSearch(board, depth - 1 + ext, -beta, -alpha, session,
                         ply + 1, move);
        if (score == VALUE_NONE) {
          board.undoMove();
          score = -VALUE_INFINITE;
    ...
    if (score > alpha) {
      ...
      if (!isCapture) {
        ...
        if (is_win(score))
          bonus += 4 * depth * depth;
        session.historyHeuristic[(int)move.from()][(int)move.to()] = std::clamp(
            session.historyHeuristic[(int)move.from()][(int)move.to()] + bonus,
            -16384, 16384);
      }
    }
...
void search(const chess::Board &board, const timeman::LimitsType timecontrol) {
  ...
  for (int i = 1; i <= timecontrol.depth; i++) {
    ...
    if (i >= 3 && prevScore != VALUE_NONE && !is_win(prevScore) &&
        !is_loss(prevScore)) {
    ...
    if (session.tm.elapsed() >= session.tm.optimum() ||
        session.tm.elapsed() >= session.tm.maximum() ||
    ...
    std::string firstMove =
        (sp == std::string::npos) ? pvStr : pvStr.substr(0, sp);
    if (!firstMove.empty())
      lastPV[0] = chess::Move(chess::uci::uciToMove(board, firstMove).raw());
    std::stringstream ss;
    ss << "qnodes " << session.qnodes << " lmrResearches "
       << session.lmrResearches << " ttHits " << session.ttHits << " ttCutoffs "
       << session.ttCutoffs << " nullCutoffs " << session.nullCutoffs;
    ...
    if (entry && entry->getMove() != Move::none().raw())
      ...
      if (moves.size()) {
        ...
        for (Move move : moves) {
          board_.doMove(move);
          Value score =
              -qsearch(board_, -VALUE_INFINITE, VALUE_INFINITE, tmpSession, 0);
          if (score > bestScore) {
            bestScore = score;
            best = move;
          }
