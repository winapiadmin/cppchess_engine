...
namespace search
{
    ...
    int16_t quiescence(chess::Position &board, int16_t alpha, int16_t beta, int ply)
    {
        ...
        if (moves.empty())
            return board.inCheck() ? -MATE(ply) : stand_pat;
        ...
        for (const auto &move : moves)
        {
            ...
            if (!(board.isCapture(move) || (givesCheck && (move.typeOf() & chess::Move::PROMOTION))))
                continue;

            if (move_ordering::see(board, move.to()) < 0)
                continue;
    ...
    int16_t alphaBeta(chess::Position &board, int16_t alpha, int16_t beta, int depth, int ply, PV *pv, int numExt = 0)
    {
        ...
        TTFlag flag = best_score >= beta ? TTFlag::LOWERBOUND :
                     (best_score <= alpha ? TTFlag::UPPERBOUND : TTFlag::EXACT);
