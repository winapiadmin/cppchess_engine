...
Value see(Board &board, Move move) {
  ...
  Bitboard attackers = board.attackers(WHITE, move.to(), occ) |
                       board.attackers(BLACK, move.to(), occ);
  Value gain[32];
  int d = 0;
...
void orderMoves(Board &board, Movelist &moves, Move ttMove, int ply,
                const engine::search::Session &session, Move prevMove) {
  Value scores[300];
  size_t n = moves.size();
